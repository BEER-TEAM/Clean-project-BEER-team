			...
			if(status==CIPHERING_OK)
						{
							...
							if(fireAlarm_node_type==getNodeType(&rx_msg))
								{
									for(int i =0 ; i<rx_msg.numberOfTLVs;i++)
									{
										...
										{
											uint32_t temp_value;
//											temp_value = (uint32_t *)rx_msg.tlv[i].value;
										...
										if(smokeSensorStatus_tag==rx_msg.tlv[i].tag)
										{
											...
											{
												door_close();//coment this line to maintain open door after fire occurs
											}
										}
...
//		int fei = (SX1276Read(REG_FEIMSB) << 8) + SX1276Read(REG_FEILSB);
//		printf("FEI: %d\r\n", fei);
